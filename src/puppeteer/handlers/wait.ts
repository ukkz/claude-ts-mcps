...
export async function handleWaitForTimeout(
  ...
  try {
    ...
    return createSuccessResponse(
      `Waited for ${actualDelay}ms (requested: ${args.delay}ms)`
    );
  } catch (error) {
    return createErrorResponse(
      `Failed to wait for timeout: ${(error as Error).message}`
    );
  }
}
...
export async function handleWaitForNavigation(
  ...
  try {
    ...
    return createErrorResponse(
      `Failed to wait for navigation: ${(error as Error).message}`
    );
  }
}
