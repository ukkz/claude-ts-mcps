...
export async function handleGetTitle(server: Server): Promise<CallToolResult> {
  try {
    ...
    return createErrorResponse(
      `Failed to get title: ${(error as Error).message}`
    );
  }
}
...
export async function handleGetUrl(server: Server): Promise<CallToolResult> {
  try {
    ...
    return createErrorResponse(
      `Failed to get URL: ${(error as Error).message}`
    );
  }
}
...
export async function handleGetContent(
  ...
  try {
    ...
    const displayContent = length > maxLength
    ...
    return createErrorResponse(
      `Failed to get content: ${(error as Error).message}`
    );
  }
}
...
export async function handleGetText(
  ...
  try {
    ...
    return createSuccessResponse(
      `Text content of ${args.selector}:\n"${text}"`
    );
  } catch (error) {
    return createErrorResponse(
      `Failed to get text: ${(error as Error).message}`
    );
  }
}
...
export async function handleGetElementInfo(
  ...
  try {
    ...
    const info = await page.evaluate((selector, includeStyles, includeAccessibility) => {
      ...
      return info;
    }, args.selector, args.includeStyles, args.includeAccessibility);
    ...
    return createErrorResponse(
      `Failed to get element info: ${(error as Error).message}`
    );
  }
}
