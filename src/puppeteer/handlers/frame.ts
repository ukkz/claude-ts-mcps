...
export async function handleGetFrames(
  ...
  try {
    ...
    return createErrorResponse(
      `Failed to get frames: ${(error as Error).message}`
    );
  }
}
...
export async function handleSwitchToFrame(
  ...
  try {
    ...
    if (!targetFrame) {
      ...
      return createErrorResponse(
        "Must specify either frameSelector, frameName, or frameIndex"
      );
    }
    ...
    return createErrorResponse(
      `Failed to switch to frame: ${(error as Error).message}`
    );
  }
}
...
export async function handleSwitchToMainFrame(server: Server): Promise<CallToolResult> {
  try {
    ...
    return createErrorResponse(
      `Failed to switch to main frame: ${(error as Error).message}`
    );
  }
}
...
export async function handleEvaluateInFrame(
  ...
  try {
    ...
    return createErrorResponse(
      `Failed to evaluate in frame: ${(error as Error).message}`
    );
  }
}
...
export async function handleSearchAcrossFrames(
  ...
  try {
    ...
    const searchInFrame = async (frame: Frame, framePath: string[]) => {
      try {
        const frameResults = await frame.evaluate((selector, text, attributes) => {
          const matches: any[] = [];
          ...
          return matches;
        }, args.selector, args.text, args.attributes);
    ...
    return createErrorResponse(
      `Failed to search across frames: ${(error as Error).message}`
    );
  }
}
