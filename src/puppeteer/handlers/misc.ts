...
export async function handleAddScriptTag(
  ...
  try {
    ...
    if (args.url) {
      ...
      return createErrorResponse("Must specify either url, path, or content for script tag");
    }
    ...
    return createErrorResponse(`Failed to add script tag: ${(error as Error).message}`);
  }
}
...
export async function handleClearInput(
  ...
  try {
    ...
    return createSuccessResponse(`Cleared input field: ${args.selector}`);
  } catch (error) {
    return createErrorResponse(`Failed to clear input: ${(error as Error).message}`);
  }
}
