...
export async function handleKeyboardPress(
  ...
  try {
    ...
    if (keys.length > 1) {
      ...
      // メインキーを押して離す
      await page.keyboard.press(keys[keys.length - 1], { delay: args.delay });
      ...
      return createSuccessResponse(
        `Pressed key combination: ${args.key}` +
        (args.delay ? ` with ${args.delay}ms delay` : '')
      );
    } else {
      // 単一キーの場合
    ...
    return createErrorResponse(
      `Failed to press key '${args.key}': ${(error as Error).message}`
    );
  }
}
